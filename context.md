...
- DEHN wide desktop cards: `desktopWideCarousel` → `.cs-cards--wide` (`aspect-ratio: 1256/650`, 2px inset); other cases square.
- Mobile nav = Modern Mobile Menu (`ui/modern-mobile-menu.tsx`, 21st.dev easemize port; `.imenu*` CSS reconstructed in `PillNav.css`). Controlled by `currentSlide`; old hamburger + popover + Escape/focus handling deleted; mobile logo hidden (hide rule must stay after `.pill-logo` base rule — cascade order). Desktop nav byte-identical.
- Harness: `mobile-shots.mjs` names fixed to current deck order; new `scripts/crop-shots.mjs` (full-res text-region crops, ~5s settle per slide).

...
- **CardSwap orphaned.** `ServicesSlide` no longer renders `CardSwap` (desktop is the ledger); `src/components/ui/CardSwap/CardSwap.jsx` remains but is unimported. Left in place pending an explicit removal decision.
- **Verification.** `npm run lint` (1 pre-existing `badge.tsx` react-refresh warning, 0 errors) + `npm run build` pass. `.github/workflows/deploy.yml` remains excluded from this push (remote PAT lacks `workflow` scope).

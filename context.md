...
- **Cleanup.** Removed dead code surfaced by this pass: the now-unused `cardWidth` prop on `ParallaxCardSlider` (+ its two call sites) and the prototyped `lockWidth` path in `WordRotate`. `npm run lint` (0 warnings) and `npm run build` pass.
...
- **Naming note:** `ParallaxCardSlider` is now a misnomer (no longer parallax) but the rename was skipped to avoid churn across its importers — it's a Blossom wrapper now.

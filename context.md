...
- **Nav glass blend-in (owner picked from three options: glass refit / editorial rail / bottom dock).** Diagnosis: the desktop bar + logo chip were already dark glass; what read out-of-place was every solid-white piece — active pill (`--base` was white), mobile popover (`white/0.85`), white hamburger circle, white backdrop. Changes: `PillNav.tsx` vars → `--base` teal `hsl(180,45%,53%)`, `--pill-bg: transparent`, `--pill-text: white/0.85`; `PillNav.css` → `.pill.is-active` explicit ink text (it used `var(--pill-bg)`, now transparent), hamburger = dark glass chip + teal lines, popover `hsla(214,30%,8%,0.85)` + white/10 hairline + white links + teal active/tap, backdrop `black/0.55`. Behavior (`Index.tsx`): `visible={isMobile ? navActive && !onCaseStudy : onCaseStudy ? navActive : true}` — desktop pinned on slides 0-3 + Contact, activity-revealed/idle-hidden on cases 4-11; mobile unchanged. Anime.js hover/show machinery, ARIA, Escape/focus wiring untouched. Verified live (desktop screenshot + mobile computed styles). **Gotcha:** `.pill.is-active` text and `.hamburger-line` previously inherited color from `--pill-bg`; any future var change must re-check both rules.

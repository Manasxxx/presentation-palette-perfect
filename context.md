...
- **Services slide ledger (desktop).** The editorial index list was reworked for scan speed in `ServicesSlide.tsx`: teal ordinal anchors `01–05` per row, title column `minmax(0,23rem)`, descriptions brightened `white/55 → white/70` (hover `/90`) and bumped to `text-lg/xl`, deliverable tags enlarged with calmer tracking, a quiet `hover:bg-white/[0.03]` row tint, and a visible `+5` counter as the HoverCard depth cue (the affordance was previously invisible). Grid is `[2.75rem_minmax(0,23rem)_minmax(0,1fr)_auto]`.
- **Case-study Split Studio v2 (desktop, Mitsui first then ported to `CaseStudyLayout`).** Stats moved from the cramped top-right pill row to a **full-width bottom proof strip** (`md:inset-x-12 md:bottom-10`, 4 columns, 900-weight numerals `clamp(1.8rem,2.3vw,2.5rem)`, muted small-caps labels, hairline top rule). Dividers are explicit per-cell `md:border-l`, NOT `divide-x` — `divide-x` loses to the cells' `md:border-0` in the cascade. The Market/Buyer/Role glass box became an open hairline ledger (no fill, no blur, `md:top-[46%]`, bigger labels/values; `lightMode` colors respected via `statBorder`/inline ternaries with desktop-vs-mobile branches). Header column widened to `md:w-[36%]`; carousel shifted right-of-center (`md:left-[64%]`, `md:top-[44%]` with stats, `md:top-[50%]` without — stats-less slides like DEHN center vertically). Mitsui desktop subtitle deduped ("Specialty chemicals across APAC" repeated the Market row; now "Specialty chemicals giant. We made technical value visible…"). Mobile pill grid, proof card, and stack carousel classes are byte-for-byte preserved in both files.
- **Who We Are (desktop) outcome consolidation.** `SkyrocketSlide.tsx` desktop `differentiators` went 5 icon-cards → 3 consolidated buyer outcomes (Understood faster / Easier internal buy-in / Lower perceived risk, fuller wording than the mobile variants), and the support paragraph was tightened + brightened (`white/70 → white/85`).
- **CardSwap deleted** (push 2). `src/components/ui/CardSwap/` (jsx + css) removed — orphaned since the desktop Services ledger replaced CardSwap; owner approved deletion.
- **iPhone ghost strip — two more attempts, STILL OPEN** (push 2). Attempt A: removed `backdrop-filter: blur(3px)` from `.cs-mobile-stack-card` (rotated, auto-transitioning, `will-change` cards — prime iOS stale-backdrop-snapshot suspect; frame tint bumped `0.045 → 0.06` to compensate). Attempt B: new `clearInlineFilter()` in `slide-motion.ts` clears the residual inline `filter: blur(0px)` that every entrance animation left behind (helpers + both case-study files), so settled elements no longer hold live filter compositor layers. **Owner re-tested on device: strip still appears.** Ruled out so far across Sessions 39–40: seam overlays, wrapper blur, mobile clip-path/circle wipe, per-card backdrop-filter, residual blur(0px) layers. Remaining suspects for next pass: the proof table / stat pill `backdrop-blur-sm` (still live on mobile), `box-shadow` on stack cards, the Motion scroll cross-fade wrapper itself, `will-change: transform, opacity` on stack cards, and the `bg-wipe` layer. Consider a forced-recomposite nudge on slide settle as a mitigation if no layer-level culprit is found.
- **Verification.** `npm run lint` (1 pre-existing `badge.tsx` react-refresh warning, 0 errors) + `npm run build` + `deck-snap` tests pass. `.github/workflows/deploy.yml` remains excluded from both pushes (remote PAT lacks `workflow` scope).
...
- **Sweep flash root-caused and fixed** (`MobileTransitionLayer.tsx`). Two mechanical flaws: (1) the 240ms idle timer cleared opacity to 0 in a single frame (hard snap-off), and (2) intensity was coupled raw to scrollTop, so the proximity snap crossing the seam in a few frames reduced the sweep to a 1–2 frame blip. Fix: dt-based eased follower — raw scroll values are only targets; displayed intensity eases toward them with fast attack (rate 14) and slow release (rate 5, ~450ms tail), and the RAF self-stops once faded below 0.004. Frame-rate independent (`1 - exp(-rate·dt)`), so 60Hz and 120Hz phones behave the same.
- **Circular glow mark removed** (`MobileTransitionLayer.tsx`, `index.css`, `theatre/deck.ts`). The centred 140px `.deck-sig-mark` read as a stray circle on the background — deleted (element, CSS, `markOpacity` Theatre param).
- **Transition redesigned as the "liquid crest"** (same files). After review the full-bleed teal wash was replaced: a thin teal hairline (1.5px, edge-faded, `.deck-sig-crest-line`) + soft radial bloom (`.deck-sig-crest-bloom`, blur via Theatre `blurPx`) **position-locked to the physical seam** (`seamY = slideH·(1-f)`), so it enters at the viewport bottom, rides the boundary with the finger, and exits at the top. Opacity keeps the eased follower; position is exact (easing it would detach the crest from the seam). Theatre object params are now `tealOpacity`/`blurPx`/`glowScale` (`travelY` removed). Owner approved on device.
- **Staggered slide entrance** (`MobileSlideMotion.tsx`, `SlideReveal.tsx`). Non-native-motion slides (Cover, Who We Are, Contact) no longer fade in as one flat block: an animejs stagger reveals top-level elements one by one (110ms apart, 560ms each, `cubicBezier(0.22,1,0.36,1)`) when the slide settles ≥50% in view, re-arming when fully offscreen so it replays per visit. `findStaggerTargets` walks down single-child wrappers to the first node with several element children (cap 12). 1.6s fallback timer guarantees content can't stay hidden. `SlideReveal` now passes `nativeMotion` into `MobileSlideMotion` so Services/Clients/case studies (which own animejs entrances) skip the stagger.
- **Color-aware seam blend** (`src/pages/slide-edge-colors.ts` new, `SlideReveal.tsx`, `Index.tsx`, `index.css`). The fixed `.deck-seam-fade` viewport overlays faded everything to the dark background — on the light case studies (Baxsaa, CTP, VNT) that dark edge tint itself read as the divider. Replaced with per-seam gradients: each slide joint blends through `color-mix(in oklab, bottomOf(A) 50%, topOf(B))`, with short 8svh gradients on both sides meeting at that same mixed color (rendered as static overlays in `SlideReveal`'s mobile branch — not motion, so not reduced-motion gated). Edge colors are eyeballed flattenings of each slide's real background; **slide-order/background changes must update `slide-edge-colors.ts`** (same bookkeeping as `Index.tsx`/`PillNav`/`DebugMenu`).
- **Verification.** `npm run lint` (0 warnings) + `npm run build` pass after each step; owner verified feel on their phone over the LAN dev server (`http://192.168.0.132:8080/`) and approved. `.github/workflows/deploy.yml` left excluded again (remote PAT lacks `workflow` scope).
...
- **Cover text reveal rebuilt** (`src/components/slides/TitleSlide.tsx`, `title-motion.ts`). `OWLSURF DIGITAL` now drops in as one lockup, so `SURF` no longer appears late. The headline, rotating pills, CTA, and badge strip use a shared Anime.js title profile with smoother cubic-bezier timing and a small spring finish.
- **Deck transition softened** (`src/components/DeckTransitionLayer.tsx`, `src/pages/deck-transition.ts`). The liquid wash/crest now uses longer mobile timings, deterministic organic drift, and gentler settle values. `getDeckTransitionMotionProfile` now includes settle duration/bounce fields, and `getDeckTransitionOrganicOffset` keeps the watery offset varied without randomness between renders.
- **Native-motion ownership** (`src/components/SlideReveal.tsx`, `src/pages/Index.tsx`). Services, Clients, and all case studies are marked `nativeMotion`, so `SlideReveal` skips the wrapper reveal and `DeckTransitionLayer` supplies only the light wash/crest instead of double-moving their content. This is the main fix for the double jaggering on mobile.
- **Buyer Systems refined** (`src/components/slides/ServicesSlide.tsx`, `src/index.css`). The mobile badge/list entry is fully Anime.js-driven. The old `sv-step-in` keyframe was removed, the rogue `BUILD ORDER` text is gone, and category observer churn was reduced with an active-key ref instead of re-creating the entry observer on every category change.
- **Clients renamed and smoothed** (`src/components/slides/ClientsSlide.tsx`). The heading is now only `CLIENTS`, with no Proof wording or proof eyebrow. The duplicate internal accent motion was removed and logo/card entry uses the shared calmer easing.
- **Case studies unified** (`src/components/slides/CaseStudySlide.tsx`, `src/components/slides/CaseStudyLayout.tsx`). Mitsui and the shared case-study slides now use the same no-double-motion path. The separate title-accent twitch was removed, carousel/subtitle entry was softened, and mobile waits for real intersection instead of using the old offscreen fallback.
- **Efficiency pass.** Removed the no-op native-slide animation in the deck transition layer, guarded optional Skyrocket targets so Anime.js is not asked to animate empty node lists, moved touched files off deprecated Anime.js string eases, and kept `.github/workflows/deploy.yml` excluded because it is an unrelated workflow-file retry change.
- **Verification before push.** Focused Vitest motion tests pass (10 tests), `npm run lint` passes, `npm run build` passes, and `git diff --check` passes. Known build notices remain: stale Browserslist/caniuse-lite and `vendor-3d` over 600 KB.

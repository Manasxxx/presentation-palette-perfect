...
## Current State (Session 44 push prep)

Case-study readability pass, DEHN wide desktop cards, mobile nav replaced with Modern Mobile Menu.

- **Case-study readability (desktop only, mobile values untouched).** `CaseStudyLayout.tsx`: `muted` (subtitle + ledger values) `white/0.84 → 0.9` dark / `0 0% 28% → 20%` light; desktop stat-strip labels `white/0.6 → 0.8` dark / `0 0% 30% → 20%` light. Same values ported to the Mitsui custom slide (`CaseStudySlide.tsx`: subtitle `text-white/85 → /90`, labels `md:text-white/60 → /80`). Root-cause find: **CTP and Baxsaa gradient end stops were translucent brand tints** (`hsl(green / 0.2)`, `hsl(maroon / 0.15)`) compositing over the dark section `bg-background` into a dark corner under the dark-ink stat strip — replaced with opaque light equivalents (`hsl(95 30% 74%)`, `hsl(8 28% 79%)`). VNT's dark corner kept (no text sits on it). `slide-edge-colors.ts` checked — existing eyeballed values already match the new opaque stops.
- **DEHN desktop wide cards.** New `desktopWideCarousel` prop chain (`DEHNCaseStudy` → `CaseStudyLayout` → `CaseStudyCarousel`) adds `.cs-cards--wide` on the desktop stack: `aspect-ratio: 1256/650` (all six DEHN creatives are that size) kills the square-card letterbox; cards keep a 2px inset (`padding: 2px`, img radius `calc(1rem - 2px)`). Other cases keep square cards; mobile untouched.
- **Mobile nav = Modern Mobile Menu (`src/components/ui/modern-mobile-menu.tsx`, NEW).** Local port of 21st.dev `easemize/modern-mobile-menu` (registry JSON had the TSX but no component CSS — `.imenu*` styles reconstructed in `PillNav.css`, `iconBounce` keyframes from the registry). Adapted controlled: `activeIndex` comes from `currentSlide` via `slideToNavIndex`, so deck scroll moves the highlight; taps call `onNavigate`. Behavior: active icon bounce (reduced-motion gated), label reveal, underline grows to the measured label width (`--lineWidth` + resize re-measure). The old mobile hamburger + popover + backdrop and their Escape/focus disclosure handling were fully deleted from `PillNav.tsx`/`PillNav.css`; the mobile logo is hidden (Cover item = home) — **the hide rule must stay after the `.pill-logo` base rule** (same specificity, cascade order decides; first attempt failed on this). Items: Cover/Positioning/Services/Proof/Cases/Contact with lucide icons. Desktop nav byte-identical (verified by screenshot); idle show/hide + mobile case-study nav hiding preserved (`visible` prop untouched, menu items joined the show/hide `movingItems` stagger).
- **Harness.** `scripts/mobile-shots.mjs` names array updated to the current 13-slide order (had the pre-S30/S35 order with ourteam/raychem ghosts). New `scripts/crop-shots.mjs`: full-res Playwright crops of case-slide text regions (claim column + stat strip) for readability checks; needs ~5s settle per slide or it catches entrance animations mid-flight.
- **Verification.** `npm run lint` (0 errors; pre-existing `badge.tsx` + `brand-marks.tsx` warnings), vitest 16/16, `npm run build` pass. `.github/workflows/deploy.yml` excluded (PAT lacks `workflow` scope).
...
### Session 43 — cover lockup + Services D2 pillar cards + brand marks + circle-wipe removal
- Cover: OWLSURF all teal + glow, DIGITAL white; tagline rebuilt as pill chip (`WHERE TECH MEETS` white/85 + `DESIGN` teal black, tracking 0.22em, `md:-ml-4` optical align); serif-italic "design" treatment gone.
- Services desktop: S40 ledger + HoverCard → five glass outcome pillar cards, 3+2 grid (Found/Seen/Understood/Trusted/Known, owner's taxonomy); `desktopServices` reshaped `{outcome, description, tags}`; mobile branch untouched (old five categories, parked).
- `brand-marks.tsx` NEW: official inline-SVG marks (Google/Meta/LinkedIn/ChatGPT/Claude/Gemini, simple-icons paths); used in Services tags + Clients "Certified partner" pill (sr-only text kept).
- Hallmark minors (Services/Clients only): dead `tabIndex` removed, −2px hover lift, ordinals /40, Clients heading `Our ` white + `Clients` teal, named-property transitions, `tabular-nums`. Open: Who We Are 3 eyebrows (cap 1-2, owner sign-off needed).
- Circle wipe removed (all 8 cases): `.bg-wipe` had been invisible since inception — full-opacity duplicate gradient at `z-[-1]` painted same-over-same (only GirlUp's translucent purple double-compounded). Made real, owner cut it as not worth the weight. One static bg div per case; if iPhone ghost strip persists, bg-wipe eliminated as suspect by absence.

...
class CarLights {
  ...
  init() {
    ...
    const instanced = new THREE.InstancedBufferGeometry().copy(geometry) as THREE.InstancedBufferGeometry;
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    // Ensure canvas fills container so clientWidth/clientHeight works
    if (myApp.renderer && myApp.renderer.domElement) {
      myApp.renderer.domElement.style.width = '100%';
      myApp.renderer.domElement.style.height = '100%';

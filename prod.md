...
- The cover (TitleSlide) is now hook-led, not wordmark-led. The hero is a bold editorial headline (`We make complex products easy to buy.`) with the accent phrase in a cursive/serif teal treatment; the `OWLSURF` wordmark + subline sits as a smaller brand lockup top-left, and the eyebrow + `01` index sits top-right. A thin teal seam runs down the left of the headline. The `See case studies` CTA is centered in the bottom zone, with the partner badge strip centered beneath it under a `Trusted partner` label. Do not reintroduce the old centered-wordmark hero or the `What We Do` / `Made For` info columns unless explicitly asked.
- The cover can carry compact credibility badges. Use actual badge image assets, keep them as a compact horizontal strip secondary to the wordmark/logo, and avoid decorative framing or per-badge frames. Badge heights are trimmed so the strip reads as one neat row.
...
- The cover slide's right-side logo slot uses the animated `OwlSurfLogo` SVG component inside the existing circular glass/ring treatment. Keep the placement and interaction shell intact when updating the mark.

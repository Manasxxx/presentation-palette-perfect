...
- The cover (TitleSlide) is now hook-led, not wordmark-led. The hero is a bold editorial headline (`We make complex products easy to buy.`) with the accent phrase in a cursive/serif teal treatment; the `OWLSURF DIGITAL` lockup sits top-left, the large clipped globe is centered behind the bottom zone with only its top half visible, and the `See case studies` CTA is centered above the partner badge strip. Do not reintroduce the old centered-wordmark hero, the `What We Do` / `Made For` info columns, or the top-right `Credentials / 01` label unless explicitly asked.

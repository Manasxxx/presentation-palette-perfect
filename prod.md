...
- Slide 2 Who We Are is a full-width editorial layout over the Hyperspeed running-lines background on desktop and a compact buyer-outcome layout on mobile. Structure is three clear blocks: top-left message (`What we understand`), top-right `Priority sectors` rail, and a full-width `What this means for buyers` outcome row/list across the bottom. The current headline is `Long sales. Technical buyers.`, with `sales.` and `buyers.` in the teal cursive serif treatment. Keep the running lines visible on desktop and keep mobile inside one viewport. Do not reintroduce the old split layout, the engineering illustration, per-row numbering, the wide rule/em-dash styling, `Why this replaces the PDF`, or the old generic `Deck to demand` phrasing unless asked.
...
- **Heavy/oversized widgets get compact mobile variants, they are not just shrunk.** The Our Team WebGL lanyard (≈680px, foreground-hero exception on desktop) is replaced on mobile by a static badge card (avatar + field/title only, no duplicate visible name) via `useIsMobile()`; the lanyard mounts on desktop only. The Services 3D `CardSwap` stack overflows phones, so mobile renders the active category's five services as a plain readable list instead, and the left pillar column becomes a horizontal-scroll chip row. Do not try to make `CardSwap` or the lanyard "fit" on mobile by scaling — use the mobile variant.

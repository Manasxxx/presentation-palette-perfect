...
- The cover (TitleSlide) is hook-led, not wordmark-led. The hero headline is `We turn [rotating industry] businesses into brands buyers actually [rotating trust word]`, with two teal dynamic pills powered by the local Anime.js `WordRotate` component. Industry words currently include Solar, Industrial, Chemical, Pharma, Manufacturing, Mobility, and Real Estate; buyer-confidence words currently include trust, choose, believe, prefer, and buy from. The `OWLSURF DIGITAL` lockup is centered on mobile and left-aligned on desktop, the right side carries an editorial signal graphic around the OwlSurf mark, and the shiny `Review case studies` CTA sits above the partner badge strip. Do not reintroduce the old centered-wordmark hero, the literal portfolio-proof info box, the `What We Do` / `Made For` info columns, the mobile credentials subline, or the top-right `Credentials / 01` label unless explicitly asked.
...
- Mobile case-study slides use a dedicated compact stack: heading + subtitle, taller Blossom cover-flow carousel, a translucent proof table, and exactly four translucent stat pills where metrics exist. Keep the `Role` row, omit the `Proof` row, and remove the least useful fifth metric instead of forcing five pills. Known unresolved bug: on the user's real phone, slides after Baxsaa can briefly appear and then turn black/dark. Next session should debug that before further polish.
...
- Our Team uses a simplified left cylindrical name roulette plus one live React Bits lanyard on desktop. The roulette and mobile badge-card swap are animated with Anime.js so active-person changes move smoothly instead of snapping. Only one WebGL lanyard should be mounted at a time; inactive names stay visually muted, the highlighted name is colored, and the roster/avatar auto-advance every 5 seconds. Do not draw the person's name on or below the lanyard; desktop lanyard and mobile badge should show the person's field/title (for example `Digital strategy`) in the primary badge text slot. The lanyard strap mark should stay upright and crisp: use a high-resolution generated band texture, React Bits-style texture repeat, and avoid stretching the OwlSurf logo artwork to compensate for MeshLine mapping. The heavy lanyard (WebGL + Rapier physics + large generated texture) is mounted on a short delay (~650ms) after the slide becomes visible, gated by a `showLanyard` state, so its initialization does not jank the scroll-snap entrance. The stage `min-h` reserves the layout height while it is deferred.
...
- Services slide (slide 4) is a left-pillars / right-CardSwap layout, NOT a flat icon grid. Five pillars use B2B buyer language: Brand & Story, Demand Gen, Discovery, Marketing Stack, AI & Autopilot. Each pillar holds 5 sub-services shown as stacked CardSwap cards. Card headings are teal; bodies stay layman-friendly while keeping industry terms (ABM, SEO, HubSpot/Marketo, AEO, Marketing Copilots). The first `Brand & Story` vertical has a two-column card body with text left and a clip-art SVG illustration right; keep the illustration contained and non-clipping. The other service verticals stay text-only unless the user asks to extend the format. The per-card `[PILLAR] • B2B / Industrial` footer band was removed in Session 11 — do not reintroduce it; the active pillar label on the left tab is already the cue. Mobile is a separate `isMobile` branch (desktop CardSwap is never touched): all five categories show as a two-row segmented icon bar (3 + centered 2, no hidden horizontal scroll), the highlighted category auto-advances every 3.5s (re-armed on change, manual tap resets, paused under reduced motion, cleared offscreen), and the active category renders as a numbered build-sequence stepper on a teal spine. The mobile stepper is titles-only and consolidated to three headline groups per category via the `mobileServices` map in `ServicesSlide.tsx`; descriptions are intentionally dropped on mobile. Item swaps fade in via the `sv-step-in` keyframe in `src/index.css`. Keep desktop showing the full five services with descriptions.
- Clients slide (slide 5) uses a top-left monster heading + centered two-row React Bits `LogoLoop` carousel. Keep true CSS mask edge fading, calm speeds, pause-on-hover, and offscreen RAF pausing. Do not reintroduce the four market-note lines below the heading unless explicitly asked.
...
- **Heavy WebGL is desktop-only by default and enforced in code.** `LightRays`, `PrismaticBurst`, and `Globe` are gated behind `useIsMobile()` and must not mount below 768px. `Hyperspeed` was temporarily re-enabled on mobile during the live Slide 2 animation pass, but it is still unreliable on the user's phone and should be revisited after the case-study black-screen issue. Gate any new heavy effect the same way unless the user explicitly asks for a mobile animation exception.
...
- **Heavy/oversized widgets get compact mobile variants, they are not just shrunk.** The Our Team WebGL lanyard (≈680px, foreground-hero exception on desktop) is replaced on mobile by a static badge card (avatar + field/title only, no duplicate visible name) via `useIsMobile()`; the lanyard mounts on desktop only. The Services 3D `CardSwap` stack overflows phones, so mobile uses a two-row segmented category bar plus an auto-advancing numbered stepper (titles only, three consolidated groups per category) instead — see the Services slide rule above. Do not try to make `CardSwap` or the lanyard "fit" on mobile by scaling — use the mobile variant.

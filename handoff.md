...
Session 36 (current) rebuilt the **mobile** deck navigation + transitions from scratch with new libraries, gated entirely behind `useIsMobile()` so **desktop is byte-for-byte unchanged**.
...
> **Next-session plan — revisit the whole mobile transition system.** The owner is not satisfied with the mobile signature transition: it flashes and doesn't "connect the journey" between slides. Come back to this entire Session-36 mobile-animation work as a focused pass. Levers to try: loosen or remove the `y proximity` snap so the crossover has room to breathe; introduce a deliberate inter-slide transition zone (extra scroll distance) the sweep can live in; re-enable Theatre studio and author the sweep as a real keyframed timeline (export → `src/theatre/deck-state.json` → load via `getProject(..., { state })`); or rethink the bridge as a shared continuity element morphing across slides. Also decide whether to finally remove `DebugMenu.tsx` and re-enable a heavy-effect mobile gate review. Everything is mobile-gated, so iterate freely without desktop risk.

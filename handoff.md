...
- **Nav glass blend-in (owner-approved direction).** The bar was already dark glass; the out-of-place pieces were the solid-white active pill, white mobile popover, white hamburger circle, and white backdrop. Now: inactive pills transparent on the bar glass (`--pill-bg: transparent`, text white/85), active pill teal with ink text, hamburger dark-glass chip with teal lines, popover dark glass (`hsl(214 30% 8% / 0.85)`) with white links + teal active, backdrop black/55. Behavior: desktop nav now pinned (always visible) on slides 0-3 + Contact; case studies stay immersive — hidden until activity, idle timer re-hides (mobile unchanged). Files: `PillNav.tsx` (color vars), `PillNav.css`, `Index.tsx` (visible expression).
